...
- **HEADER_BLOCK_LIST** (optional): comma separated list of header names useful to prevent them from being forwarded to the target service. These headers will prevail if same headers are specified on other Environment Variables
- **HEADERS_TO_REMAP** (optional): comma separated list of `key:value` pairs where `key` is the Key of a header in the incoming request and `value` is the Key of the header in the proxied request.
- **DISALLOWED_RESPONSE_CONTENT_TYPE_LIST** (optional): comma separated list of values for the `Content-Type` header that the are disallowed in the response of the target service
- **ALLOWED_PROXY_TARGET_URLS** (optional): comma separated list of base URLs. Only proxies with targetBaseUrl specified in this list can be created and handled. If none is specified all target URLs are allowed.
- **TOKEN_PREEMPTIVE_EXPIRY_SECONDS** (optional, default to `30`): number of seconds before the actual token expiration to preemptively treat the token as expired and request a new one. This prevents race conditions where a token expires between the moment it's validated and when it's actually used in the HTTP request. For example, with a 30-second preemptive expiry, a token expiring at 12:00:30 will be considered expired at 12:00:00, forcing a token refresh and avoiding 401 errors.
...
Path parameter can be specified with the `{param-name}` syntax and eventually forwarded to the external service in the *targetBaseUrl* using the same name (**only for static configuration**).
...
In order to configure correctly the CRUD collection, you can **import** the fields from this <a download target="_blank" href="docs_files_to_download/crud.fields.json">file</a>. This file already enables the Client Side Field Level Encryption (CSFLE) for those fields with sensitive data.
